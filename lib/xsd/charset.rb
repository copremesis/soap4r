...
public
  ...
  def Charset.charset_label(encoding)
    CharsetMap[encoding.to_s.upcase]
  end

  def Charset.charset_str(label)
    if CharsetMap.respond_to?(:key)
      CharsetStrCache[label] ||= CharsetMap.key(label.to_s.downcase) || 'X_UNKNOWN'
    else
      CharsetStrCache[label] ||= CharsetMap.index(label.to_s.downcase) || 'X_UNKNOWN'
    end
  end
